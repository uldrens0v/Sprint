<template>
  <div class="formulario-dinamico">
    <h1>Nuevo {{ endpointElegido.replace(/s$/, '') }}</h1>

    <form v-if="cabeceras.length > 0" @submit.prevent="insertar">
      ...
      <button type="submit">
        Insertar un {{ endpointElegido.valueOf().replace(/s$/, '') }} en la API
      </button>
    </form>
...
import { datosJSON, endpointElegido, obtenerDatos } from '@/DatosJSON.js'

const mensaje = ref('')
const form = ref({})

//intentamos obtener las cabeceras del json para ponerlas en el formulario como campos
const cabeceras = computed(() => {
  if (datosJSON.value && datosJSON.value.length > 0) {
...
//insertamos en la api al darle al submit
const insertar = async () => {
  const API_URL = `http://100.52.46.68:3000/${endpointElegido.value}`
  ...
  try {
    ...
    if (response.ok) {
      mensaje.value = `¡Insertado en ${endpointElegido.value}!`
      inicializarFormulario()
      await obtenerDatos()
    } else {
